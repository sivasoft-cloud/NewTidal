...
async function viewDetails(type, id) {
    ...
    try {
        ...
        if (type === 's') {
            details = await fetchWithCORS(`${API_BASE}/track/?id=${id}`);
        } else if (type === 'al') {
            details = await fetchWithCORS(`${API_BASE}/album/?id=${id}`);
        } else {
            throw new Error(`Unsupported type: ${type}`);
        }
        
        if (!details.ok) {
            throw new Error(`HTTP error! status: ${details.status}`);
        }
        
        const detailsData = await details.json();
        coverUrl = await getCoverImage(id);
        displayDetailView(type, detailsData, coverUrl);
    } catch (error) {
        displayError(error.message);
    } finally {
        showLoading(false);
...
async function getCoverImage(id) {
    const response = await fetchWithCORS(`${API_BASE}/cover/?id=${id}`);
    if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
    }
    const data = await response.json();
